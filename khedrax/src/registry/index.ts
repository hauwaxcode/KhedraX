...
import { listInterfaces } from './interfaceRegistry.ts';
...
export async function getRegistrySnapshot(rootDir: string, pluginRoots: string[] = []): Promise<RegistrySnapshot> {
  ...
  const [agentTypes, modules, personas, memoryBackends, deployments, interfaces] = await Promise.all([
    ...
    listInterfaces(rootDir, normalizedPluginRoots),
  ]);
  return { agentTypes, modules, personas, memoryBackends, deployments, interfaces };
}
